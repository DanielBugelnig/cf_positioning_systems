...
% Plotting the Tracks
T = 1/100;
t = T:T:T*length(x_o_15_sync);
figure(2);
subplot(3,1,1);
plotting(t,x_o_15_sync,x_l_15,"x [m]", "Comparison height 1.5m x-axis")
subplot(3,1,2);
plotting(t,y_o_15_sync,y_l_15,"y [m]", "Comparison height 1.5m y-axis")
subplot(3,1,3);
plotting(t,z_o_15_sync,z_l_15,"z [m]", "Comparison height 1.5m z-axis")

%Error
% measured value (loco) - real value (optitrack)
error_x = x_l_15 - x_o_15_sync;
error_y = y_l_15 - y_o_15_sync;
error_z = z_l_15 - z_o_15_sync;
...
x_l = x_l_15 + offset_x_optimized;
y_l = y_l_15 + offset_y_optimized;
z_l = z_l_15 + offset_z_optimized;
x_o = x_o_15_sync;
y_o = y_o_15_sync;
z_o = z_o_15_sync;
...
figure(1);
subplot(3,1,1);
plotting(t,x_o,x_l,"x[m] optimized", "Comparison at height 1.5m, x-axis")
subplot(3,1,2);
plotting(t,y_o,y_l,"y[m] optimized", "Comparison at height 1.5m, y-axis")
subplot(3,1,3);
plotting(t,z_o,z_l,"z[m] optimized", "Comparison at height 1.5m, z-axis")
